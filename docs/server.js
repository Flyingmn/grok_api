...
// 检查必填
['COOKIE','X_MS_TOKEN','DEVICE_ID','TEA_UUID','WEB_ID','MS_TOKEN','A_BOGUS','ROOM_ID','FP','WEB_TAB_ID']
...
const getQueryParams = () => {
  return `aid=497858&device_id=${CONFIG.DEVICE_ID}&device_platform=web&fp=${CONFIG.FP}&language=zh&pc_version=${CONFIG.PC_VERSION || '2.42.2'}&pkg_type=release_version&real_aid=497858&region=CN&samantha_web=1&sys_region=CN&tea_uuid=${CONFIG.TEA_UUID}&use-olympus-account=1&version_code=20800&web_id=${CONFIG.WEB_ID}&web_tab_id=${CONFIG.WEB_TAB_ID}&msToken=${CONFIG.MS_TOKEN}&a_bogus=${CONFIG.A_BOGUS}`;
};
...
const getHeaders = () => {
  return {
    ...
    'x-flow-trace': '04-00053e0824db98e7000d5e131a07e38b-000c3e5556a878ce-01'
  };
};
...
async function generateImage(promptText) {
  ...
  const body = {
    ...
    completion_option: {
      ...
      is_delete: false,
      message_from: 0,
      action_bar_skill_id: 3, // 图片生成技能ID
      use_auto_cot: false,
    ...
    section_id: "26810775915980802", // 从抓包数据中获取
    conversation_id: CONFIG.ROOM_ID,
                ...
                if (msg?.content_type === 2074) {
                  ...
                  for (const creation of content.creations || []) {
                    ...
                    if (creation?.image?.status === 2) {
                      // 优先级：image_ori_raw > image_ori > image_preview > image_thumb
                      const url = creation.image.image_ori_raw?.url || 
