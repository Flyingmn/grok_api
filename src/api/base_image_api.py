...
class BaseImageGenerator(ABC):
    ...
    async def initialize(self):
        ...
        try:
            logger.info(f"初始化{self.service_name}图片生成器（多实例模式）...")
            ...
            logger.success(f"{self.service_name}图片生成器初始化完成，当前有 {len(running_instances)} 个可用实例")
            return True
            
        except Exception as e:
            logger.error(f"初始化{self.service_name}图片生成器失败: {e}")
            return False
    ...
    async def generate_image(self, prompt: str, reference_images_b64: Optional[List[str]] = None, aspect_ratio: str = "Auto") -> Dict[str, Any]:
        ...
        try:
            ...
            # 调用具体实现的生成方法
            result = await self._generate_image_impl(client, prompt, reference_images_b64, aspect_ratio, task_id)
    ...
    async def _wait_for_response(self, client, timeout: int = 300) -> Dict[str, Any]:
        """等待AI响应并解析结果"""
        try:
            # 等待响应（默认最多5分钟）
    ...
    async def cleanup(self):
        """清理资源"""
        try:
            logger.info(f"{self.service_name}图片生成器资源清理完成")
        except Exception as e:
            logger.error(f"清理资源失败: {e}")


def create_image_api_app(generator: BaseImageGenerator, title: str, description: str) -> FastAPI:
    """创建图片生成API应用"""
    ...
    async def lifespan(app: FastAPI):
        """应用生命周期管理"""
        # 启动时执行
        logger.info(f"启动{generator.service_name}图片生成API服务（多实例模式）...")
        ...
        # 关闭时执行
        logger.info(f"正在关闭{generator.service_name}图片生成API服务...")
    ...
    async def root():
        """根路径"""
        return {
            "message": f"{generator.service_name} 图片生成API服务",
            "version": "1.0.0",
            "status": "running"
        }
