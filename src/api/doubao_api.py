...
class DoubaoImageGenerator(BaseImageGenerator):
    ...
    def __init__(self):
        super().__init__("豆包", doubao_browser_manager)
    ...
    async def _generate_image_impl(self, client, prompt: str, reference_images_b64: Optional[List[str]], aspect_ratio: str, task_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if "https://www.doubao.com/chat/" not in current_url:
                logger.info("当前不在豆包聊天页面，开始导航...")
                if not await client.navigate_to_doubao():
                    logger.warning("导航到豆包页面失败，但继续执行")
            else:
                logger.info("已在豆包聊天页面，跳过导航")
            ...
            # 等待并解析响应 - 豆包专用处理
            logger.info("等待豆包AI响应...")
            result = await self._wait_for_doubao_response(client, timeout=300)
            
            return result
            
        except Exception as e:
            logger.error(f"豆包生成图片时出错: {e}")
            return {
                "success": False,
    ...
    async def _cleanup_after_task(self, client):
        ...
        try:
            # 豆包特定的清理逻辑
            # 豆包客户端暂时没有特定的清理方法，可以在这里添加
            logger.info("豆包任务清理完成")
        except Exception as e:
            logger.warning(f"豆包任务清理失败: {e}")
    ...
    async def _upload_single_image(self, client, image_path: str) -> bool:
        ...
        try:
            ...
            logger.error(f"上传图片到豆包失败: {e}")
            return False
    
    async def _wait_for_doubao_response(self, client, timeout: int = 300) -> Dict[str, Any]:
        """等待豆包AI响应并解析结果"""
        try:
            # 等待响应（默认最多5分钟）
            ...
            return await self._parse_doubao_response(latest_response)
            
        except Exception as e:
            logger.error(f"等待豆包响应时出错: {e}")
            return {
                "success": False,
                "message": f"等待豆包响应时出错: {str(e)}"
            }
    
    async def _parse_doubao_response(self, api_response: Dict[str, Any]) -> Dict[str, Any]:
        """解析豆包响应，提取图片URL并转换为base64"""
        try:
            logger.info("开始解析豆包生成响应...")
            ...
            if not image_urls and not ai_text:
                return {
                    "success": False,
                    "message": "豆包响应中没有图片或文本内容"
                }
            
            # 下载图片并转换为base64
            generated_images = []
            if image_urls:
                logger.info(f"豆包生成了 {len(image_urls)} 张图片，开始下载转换...")
            ...
            logger.error(f"解析豆包生成响应失败: {e}")
            return {
                "success": False,
                "message": f"解析豆包响应失败: {str(e)}"
            }
...
app = create_image_api_app(
    generator=generator,
    title="豆包 图片生成API",
    description="基于豆包的图片生成自动化服务"
)


def signal_handler(signum, frame):
    """信号处理器"""
    logger.info(f"收到信号 {signum}，正在关闭豆包服务...")
    sys.exit(0)
...
if __name__ == "__main__":
    ...
    logger.remove()
    logger.add(
        "data/logs/doubao_api_{time:YYYY-MM-DD}.log",
        rotation="1 day",
        retention="7 days",
        level="INFO",
    ...
    try:
        ...
        uvicorn.run(
            "doubao_api:app",
            host="0.0.0.0",
            port=8814,  # 使用不同的端口避免冲突
            reload=False,  # 关闭reload避免复杂的进程管理
            log_level="info"
        )
    except KeyboardInterrupt:
        logger.info("豆包服务被用户中断")
    except Exception as e:
        logger.error(f"豆包服务运行失败: {e}")
    finally:
        logger.info("豆包服务已退出")
